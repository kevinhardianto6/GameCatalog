...
class FavouriteViewController: BaseViewController {
  ...
  func setViewModel(viewModel: FavouriteViewModelProtocol) {
  ...
  func setupBinding() {
    ...
    }).disposed(by: favouriteViewModel?.disposeBag ?? disposeBag)
    
    favouriteViewModel?.success.bind(to: collectionView.rx.items(cellIdentifier: cellIdentifier, cellType: FavouriteCell.self)) { (row, game, cell) in
    ...
    favouriteViewModel?.detail.subscribe(onNext: { game in
      let detail = self.favouriteViewModel?.makeDetailView(game: game) ?? UIViewController()
